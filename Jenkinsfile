pipeline {
    ...
    stages {
        ...
        stage('Run') {
            steps {
                sh 'echo Running...'
                sh 'docker run --name worldofgames --detach --rm --publish 8777:8777 --env FLASK_APP=WorldOfGames --env FLASK_RUN_HOST=0.0.0.0 --env FLASK_RUN_PORT=8777 omritz243/worldofgames:1.0'
                sh 'docker ps -f "name=worldofgames"'
            }
        }
        stage('Test') {
            steps {
                sh 'echo Testing...'
                sh 'docker exec -i worldofgames sh -c "python WorldOfGames/e2e.py"'
            }
        }
        stage('Finalize') {
            environment {
